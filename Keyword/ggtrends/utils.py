...
def find_between(s: str, first: str, last: str):
    if not all([isinstance(s, str), isinstance(first, str), isinstance(last, str)]):
        return ""
    try:
...
def todayTrend(request):
    ...
    for item in today_trend:
        ...
        if per > 0:
            item["exploreLink"] = urllib.parse.unquote(
                find_between(
                    item["exploreLink"], "/trends/explore?q=", "&date=now+7-d&geo=VN"
                ).replace("+", " ")
            )

        else:
            item["exploreLink"] = find_between(
                item["exploreLink"], "/trends/explore?q=", "&date=now+7-d&geo=VN"
            ).replace("+", " ")
